...
def is_c_hundred(lm):
    ...
    thumb_curve = lm[4].y > lm[8].y
    return four_fingers and thumb_curve

# ====== GESTURE 0 (BARU - UNTUK 20,30,40,...,90) ======
def is_zero_gesture(lm):
    """4 jari tertutup + thumb agak terbuka = 0 (konfirmasi puluhan)"""
    tips = [8,12,16,20]  # I,M,R,P tips
    pips = [6,10,14,18]  # Pips
    all_fingers_closed = all(lm[tip].y > lm[pip].y for tip,pip in zip(tips,pips))
    thumb_up = is_thumb_up(lm)
    return all_fingers_closed and thumb_up
...
def gesture_to_number(s, lm):
    ...
    # GESTURE 0 BARU! (konfirmasi puluhan)
    if is_zero_gesture(lm):
        return 0
        ...
        for hand_landmarks, handedness in zip(
            ...
            if handedness.classification[0].label == "Right":
                ...
                if hold_frames > 0:
                    ...
                    # ===== digit → 👍 → 0 = 10,20,30,...,90 (BARU!) =====
                    elif waiting_last_digit and stable_number == 0:
                        display_number = pending_tens * 10 + 0
                        number = display_number
                        hold_frames = 35
                        waiting_last_digit = False
                        pending_tens = None
                    ...
                    # ===== digit → 👍 → digit = 11-99 =====
                        ...
                        display_number = pending_tens * 10 + stable_number
                        number = display_number
                        hold_frames = 35
                        waiting_last_digit = False
                        pending_tens = None

                    # ===== Trigger tens mode: digit → 👍 =====
