...
def detect_operator(right_lm, left_lm=None):
    ...
    left_i_up_m = left_lm[8].y < left_lm[6].y
    left_m_up_m = left_lm[12].y < left_lm[10].y
    right_far = right_lm[8].x > left_lm[8].x + 0.12
    ...
    # / PEMBAGIAN: Telapak kiri atas + kanan tegak lurus
    ...
    right_thumb_up = right_lm[4].x < right_lm[20].x
    ...
    right_i_up_clear = right_lm[8].y < right_lm[6].y
...
while True:
    ...
    if result != 0:
        cv2.putText(frame, f"{num1}{op}{num2}={result}", (10,350), cv2.FONT_HERSHEY_SIMPLEX, 3, (0,255,0), 4)
