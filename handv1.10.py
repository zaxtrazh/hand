...
def is_c_hundred(lm):
    ...
    thumb_curve = abs(lm[4].x - lm[8].x) > 0.18
        ...
        for hand_landmarks, handedness in zip(
            ...
            if handedness.classification[0].label == "Right":
                ...
                if hold_frames > 0:
                    ...
                    # ===== 1 → C = 100 =====
                    if base_number == 100 and last_stable_number == 1:
                        display_number = 100
                        number = display_number
                        hold_frames = 40

                    # ===== digit → 👍 → digit =====
                    elif waiting_last_digit and 1 <= base_number <= 9:
                        display_number = pending_tens * 10 + base_number
                        number = display_number
                        hold_frames = 35
                        waiting_last_digit = False
                        pending_tens = None

                    elif base_number == 10 and 1 <= last_stable_number <= 9:
