...
result = 0
calc_display = ""
math_step = 0  # 0=wait op1, 1=wait op2, 2=operator, 3=calculate
...
# ====== CLEAR GESTURE ======
def gesture_clear(status):
    T,I,M,R,P = status
    return T==1 and I==1 and M==0 and R==0 and P==0  # Telunjuk + Jempol

# ====== MATH OPERATOR GESTURES ======
...
def detect_multiplication(right_lm, left_lm):
    ...
    left_palm_right = left_lm[17].x > left_lm[5].x
...
def detect_division(right_lm, left_lm):
    left_palm_up = left_lm[0].z < left_lm[9].z
    right_vertical = abs(right_lm[8].x - right_lm[5].x) < 0.08
    right_across = right_lm[8].y < left_lm[0].y + 0.1
...
def detect_equals(lm):
    ...
    middle_fingers_down = (lm[8].y > lm[6].y and 
                          lm[12].y > lm[10].y and
                          lm[16].y > lm[14].y)
...
while True:
    ...
    cv2.putText(frame, f"Kamus BIMA | Calculator | DPPM Kemdiktisaintek 2026", (10,20),
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        ...
        # ====== CLEAR GESTURE (prioritas tertinggi) ======
        if hand_count == 1:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                if handedness.classification[0].label == "Right":
                    lm = hand_landmarks.landmark
                    status = finger_status(lm)
                    if gesture_clear(status):
                        digit_list.clear()
                        current_operator = None
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                if handedness.classification[0].label == "Right":
                    ...
                    if number != 0 and hold_frames == 0:
                        ...
                        if not gesture_clear(status) and number > 0 and len(digit_list) < 2:
                            digit_list.append(number)
                            math_step += 1
                            cv2.putText(frame, f"Digit {len(digit_list)} OK!", (200,280), 
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)

        # ====== 2 TANGAN = OPERATOR ======
        elif hand_count == 2 and math_step == 1:
            calc_mode = True
            ...
            if right_hand is not None and left_hand is not None and current_operator is None:
                ...
                if current_operator:
                    op1 = digit_list[0]
                    calc_display = f"{op1} {current_operator} _ = ?"
                    cv2.putText(frame, "1 Tangan: Input angka kedua", (150,320), 
        ...
        if hand_count == 1 and current_operator and len(digit_list) == 2:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                if handedness.classification[0].label == "Right":
                    lm = hand_landmarks.landmark
                    if detect_equals(lm):
                        op1, op2 = digit_list[0], digit_list[1]
                        if current_operator == "+":
                            result = op1 + op2
    ...
    # ====== DISPLAY ======
    cv2.putText(frame, f"Mode: {'MATH' if calc_mode else 'DIGIT'} | Step: {math_step}", (5,40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
    cv2.putText(frame, f"Digits: {digit_list} | Op: {current_operator}", (5,60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
    cv2.putText(frame, f"Result: {calc_display}", (5,80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
    cv2.putText(frame, f"Number: {number}", (5,100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
    
    # Big display
    if calc_display and "RESULT" in calc_display:
        cv2.putText(frame, calc_display, (100,320), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,255,0), 3)
    elif number > 0:
        cv2.putText(frame, f"{number}", (250,350), cv2.FONT_HERSHEY_SIMPLEX, 5, (0,255,0), 10)
    
    # Instructions
    cv2.putText(frame, "CLEAR: Thumb+Index | = : Thumb+Pinky", (10,340),
